...
const PINS_KEY = 'lmr:pins';

// Read the per-person edit passcodes, or null if not set up yet.
export async function getPins() {
  try {
    const v = await redisCmd(['GET', PINS_KEY]);
    if (!v) return null;
...
// Save the per-person edit passcodes (one-time setup).
export async function savePins(pins) {
  await redisCmd(['SET', PINS_KEY, JSON.stringify(pins)]);
  return pins;
}
